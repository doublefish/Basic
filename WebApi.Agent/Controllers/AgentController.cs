...
namespace WebApi.Agent.Controllers
{
	...
	public class AgentController : ApiController
	{
		...
		{
			var data = new Basic.Model.Agent()
			{
				Code = model.Code,
				Name = model.Name,
				Status = model.Status,
		...
		[ApiAuthorize(VerifyToken = true, VerifyRight = true)]
		[HttpGet("Get/{id}")]
		public ReturnResult<Basic.Model.Agent> Get(int id)
		{
		...
		[ApiAuthorize(VerifyToken = true, VerifyRight = true)]
		[HttpGet("List")]
		public ReturnResult<AgentArg<Basic.Model.Agent>> List(string name = null, int? level = null,
		...
		{
			var arg = new AgentArg<Basic.Model.Agent>(pageNumber, pageSize, sortName, sortType)
			{
				Name = name,
				Level = level,
				Status = status,
