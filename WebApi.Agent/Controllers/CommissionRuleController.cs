...
namespace WebApi.Agent.Controllers
{
	...
	public class CommissionRuleController : ApiController
	{
		...
		{
			var datas = new List<AgentCommissionRule>();
			var _productIds = productIds.Split(',');
			foreach (var productId in _productIds)
			{
				datas.Add(new AgentCommissionRule()
				{
			...
			new AgentCommissionRuleBLL(LoginInfo).Add(datas);
		...
		public ReturnResult<string> Update(int id, [FromBody] CommissionRuleModel model)
		{
			...
			new AgentCommissionRuleBLL(LoginInfo).Update(data);
			return Ok();
		}
		...
		{
			new AgentCommissionRuleBLL(LoginInfo).Enable(id);
			return Ok();
		}
		...
		{
			new AgentCommissionRuleBLL(LoginInfo).Delete(id);
			return Ok();
		}
		...
		public ReturnResult<AgentCommissionRule> GetLastest(int productId)
		{
			var result = new AgentCommissionRuleBLL(LoginInfo).GetLastest(productId);
			return Json(result);
		}
		...
		public ReturnResult<AgentCommissionRule> GetByProductId(int productId, int year, int month)
		{
			var result = new AgentCommissionRuleBLL(LoginInfo).Get(productId, year, month);
			return Json(result);
		}
		...
		/// <returns></returns>
		[ApiAuthorize(VerifyToken = true, VerifyRight = true)]
		[HttpGet("GetMine/{productId}")]
		public ReturnResult<CommissionRule> GetMine(int productId, int year, int month)
		{
		...
		public ReturnResult<CommissionRuleArg<AgentCommissionRule>> List(int? productId = null,
